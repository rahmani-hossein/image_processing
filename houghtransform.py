...
def draw_lines(image, binary_image, accumulator_matrix, rhos, cos_thetas, sin_thetas, delete=True, threshhold=160,
               dist=10, filter_length=15):
    height = image.shape[0]
    width = image.shape[1]
    gray_image = cv.cvtColor(image, cv.COLOR_BGR2GRAY)
    gray_image = gray_image.astype(np.float64)
    linedetector_filter_ver = np.ones((filter_length, 1))
    linedetector_filter_ver[:filter_length // 2, 0] = -linedetector_filter_ver[
                                                       :filter_length // 2, 0]
    linedetector_filter_ver[filter_length // 2, 0] = 0
    linedetector_filter_hor = np.ones((1, filter_length))
    linedetector_filter_hor[0, :filter_length // 2] = -linedetector_filter_hor[0,
                                                       :filter_length // 2]
    linedetector_filter_hor[0, filter_length // 2] = 0
    ver = cv.filter2D(src=gray_image, ddepth=-1, kernel=linedetector_filter_ver)
    hor = cv.filter2D(src=gray_image, ddepth=-1, kernel=linedetector_filter_hor)
    ...
    for p in range(sd.shape[0]):
        ...
        if delete != True:
            ...
            for x_line in range(np.minimum(x1, x2), np.maximum(x1, x2), 1):
                for y_line in range(np.minimum(y1, y2), np.maximum(y1, y2), 1):
                    num=0
                    if binary_image[x_line, y_line] > 0 and ver[x_line, y_line] > 900 and hor[x_line, y_line] > 900:
                        num=num+1
            if num> 20:
                cv.line(image, (y1, x1), (y2, x2), (0, 0, 255), thickness=3)
    return image
...
def find_onepixel_edge(binary_image, morphology_len):
    edge_image = cv.dilate(binary_image, cv.getStructuringElement(cv.MORPH_RECT, (morphology_len, morphology_len)),
                           iterations=1)
    edge_image = cv.erode(edge_image, cv.getStructuringElement(cv.MORPH_RECT, (morphology_len, morphology_len)),
                          iterations=1)
    return edge_image
...
lined_image1 = draw_lines(img1.copy(), edges1, accomulator1, rhos1, cos_thetas1, sin_thetas1, False, 120, 10)
cv.imwrite('res05-lines.jpg', lined_image1)
binary_image1 = find_onepixel_edge(edges1, 5)
...
lined_chess1 = draw_lines(img1.copy(), binary_image1, accomulator1, rhos1, cos_thetas1, sin_thetas1, True, 120, 10)
